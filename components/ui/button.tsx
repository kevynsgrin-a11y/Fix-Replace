...
const buttonVariants = cva(
  ...
  ],
  {
    variants: {
      ...
      size: {
        sm: "h-8 rounded-(--radius-xs) px-3 text-(length:--text-xs)",
        md: "h-10 rounded-(--radius-sm) px-4 text-(length:--text-sm)",
        lg: "h-12 rounded-(--radius-md) px-6 text-(length:--text-base)",
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, block, href, type = "button", ...props }, ref) => {
