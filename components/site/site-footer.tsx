      ...
      <Container wide className="py-14">
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 nav:grid-cols-[1.4fr_repeat(3,1fr)]">
          ...
          <div className="flex flex-col gap-4">
            ...
            >
              <LogoMark />
              <span className="text-(length:--text-base) font-semibold tracking-[-0.02em] text-(--color-ink)">
                Repair<span className="text-(--color-muted)">Or</span>Replace
              </span>
            </Link>
            <p className="max-w-xs text-(length:--text-sm) text-(--color-muted)">
          ...
          {footerNav.map((col) => (
            <nav key={col.heading} aria-label={col.heading}>
              <h2 className="text-(length:--text-2xs) font-semibold uppercase tracking-[0.08em] text-(--color-muted)">
                {col.heading}
              </h2>
              <ul className="mt-4 flex flex-col gap-2.5">
                {col.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="inline-flex rounded-[3px] text-(length:--text-sm) text-(--color-body) transition-colors [transition-duration:var(--duration-fast)] hover:text-(--color-ink)"
                    >
                      {link.label}
                    </Link>
                  </li>
