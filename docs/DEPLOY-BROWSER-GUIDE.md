...
>   to upload.
> - The canonical domain is **`https://www.repair-or-replace.net`** — the `www` host,
>   because the apex 308-redirects to it. `repair-or-replace.com` is **not**
>   attached to the Vercel project and returns 404, so it must not be made
>   canonical until that changes. The canonical origin is configured centrally
>   in [`../lib/site.ts`](../lib/site.ts).
...
- **Submitting the sitemap** to Google Search Console after launch, now at
  `https://www.repair-or-replace.net/sitemap.xml`. The sitemap is generated by
  `app/sitemap.ts`.
